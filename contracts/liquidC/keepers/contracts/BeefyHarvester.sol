...
import "../interfaces/ILiquidCVault.sol";
import "../interfaces/ILiquidCStrategy.sol";
import "../interfaces/ILiquidCRegistry.sol";
import "../interfaces/ILiquidCHarvester.sol";
import "../interfaces/IUpkeepRefunder.sol";

import "../libraries/UpkeepLibrary.sol";

contract LiquidCHarvester is ManageableUpgradeable, ILiquidCHarvester {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Contracts.
    ILiquidCRegistry public _vaultRegistry;
    ...
    uint256 public _vaultHarvestFunctionGasOverhead; // Estimated average gas cost of calling harvest(). TODO: this needs to live in LiquidCRegistry, and needs to be a `per vault` number.
    ...
    function initialize(
        ...
        _vaultRegistry = ILiquidCRegistry(vaultRegistry_);
        ...
        )
    {
        ...
        if (numberOfVaultsToHarvest == 0) return (false, bytes("LiquidCAutoHarvester: No vaults to harvest"));
    ...
    function _canHarvestVault(address vaultAddress_) internal view returns (bool canHarvest_) {
        ILiquidCVault vault = ILiquidCVault(vaultAddress_);
        ILiquidCStrategy strategy = ILiquidCStrategy(vault.strategy());
        ...
        )
    {
        ...
        uint256 vaultHarvestGasOverhead = _estimateSingleVaultHarvestGasOverhead(_vaultHarvestFunctionGasOverhead); // TODO: Pull this number from LiquidCRegistry.
    ...
    function _harvestVault(address vault_) internal returns (bool didHarvest_, uint256 callRewards_) {
        ILiquidCStrategy strategy = ILiquidCStrategy(ILiquidCVault(vault_).strategy());
