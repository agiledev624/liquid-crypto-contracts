...
import "../interfaces/ILiquidCStrategyEthCall.sol";
...
contract VaultGasOverheadAnalyzer is ManageableUpgradeable, KeeperCompatibleInterface {
    ...
    ILiquidCRegistry private _vaultRegistry;
        ...
        )
    {
        ...
        ILiquidCVault vaultToAnalyze = ILiquidCVault(vaults[currentIndex]);
    ...
    function _analyzeHarvest(ILiquidCVault vault) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        ILiquidCStrategyEthCall strategy = ILiquidCStrategyEthCall(address(ILiquidCVault(vault).strategy()));
    ...
    function _tryHarvest(ILiquidCStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
    ...
    function _tryOldHarvest(ILiquidCStrategyEthCall strategy) internal view returns (bool didHarvest_, uint256 gasOverhead_) {
        uint256 gasBefore = gasleft();
