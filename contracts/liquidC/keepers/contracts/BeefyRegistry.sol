...
contract LiquidCRegistry is ManageableUpgradeable, ILiquidCRegistry {
    ...
    function _addVault(address _vaultAddress) internal {
        ...
        ILiquidCVault vault = ILiquidCVault(_vaultAddress);
        ILiquidCStrategy strat = _validateVault(vault);
    ...
    function _validateVault(ILiquidCVault _vault) internal view returns (ILiquidCStrategy strategy) {
        address vaultAddress = address(_vault);

        try _vault.strategy() returns (ILiquidCStrategy _strategy) {
            require(ILiquidCStrategy(_strategy).vault() == vaultAddress, "Vault/Strat Mismatch");
            return ILiquidCStrategy(_strategy);
        } catch {
            require(false, "Address not a Vault");
        }
    }

    function _collectTokenData(ILiquidCStrategy _strategy) internal view returns (address[] memory tokens) {
        ...
        )
    {
        ...
        name_ = vault.name();
        strategy_ = ILiquidCStrategy(vault.strategy());
    ...
    function getStakedVaultsForAddress(address _address) external view returns (VaultInfo[] memory stakedVaults) {
        ...
        for (uint256 vid; vid < _vaultSet.length(); vid++) {
            if (ILiquidCVault(_vaultSet.at(vid)).balanceOf(_address) > 0) {
                numResults++;
            }
        }
