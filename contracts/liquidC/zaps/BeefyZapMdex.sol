...
// @author Wivern for LiquidC.Finance
...
interface ILiquidCVault is IERC20 {
    function deposit(uint256 amount) external;
    function withdraw(uint256 shares) external;
    function want() external pure returns (address); // LiquidC Vault V6
    function token() external pure returns (address); // LiquidC Vault V5
}

contract LiquidCZapMdex {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    using SafeERC20 for ILiquidCVault;
    ...
    function beefInETH (address LiquidCVault, uint256 tokenAmountOutMin) external payable {
    ...
    function beefIn (address LiquidCVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
    ...
    function beefOut (address LiquidCVault, uint256 withdrawAmount) external {
        (ILiquidCVault vault, IUniswapV2Pair pair) = _getVaultPair(LiquidCVault);
    ...
    function beefOutAndSwap(address LiquidCVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        (ILiquidCVault vault, IUniswapV2Pair pair) = _getVaultPair(LiquidCVault);
    ...
    function _getVaultPair (address LiquidCVault) private view returns (ILiquidCVault vault, IUniswapV2Pair pair) {
        vault = ILiquidCVault(LiquidCVault);
